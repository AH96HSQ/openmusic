...
interface SyncData {
  ...
  playlists: any[];
  recommendations?: any[];
  recommendationsTimestamp?: string;
  syncedAt: string;
}
...
class SyncController {
   ...
   */
  async uploadSyncData(userId: string, data: SyncData, clientTimestamp?: number): Promise<number> {
    ...
    await UserSyncData.findOneAndUpdate(
      { userId },
      {
        userId,
        songs,
   ...
   */
  async smartSync(userId: string, clientTimestamp: number, clientData?: SyncData): Promise<SmartSyncResponse> {
    const syncData = await UserSyncData.findOne({ userId });
   ...
   */
  async deleteSyncData(userId: string): Promise<void> {
