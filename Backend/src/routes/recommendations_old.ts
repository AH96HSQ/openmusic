...
router.post("/similar", async (req: Request, res: Response) => {
  try {
    const { coreSongs, dislikedSongIds = [], sessionId } = req.body as RecommendationRequest & { sessionId?: string };
    ...
    console.log(
      `[Recommendations] Processing ${coreSongs.length} core songs for recommendations (session: ${sessionId || "none"})`
    );
    ...
    const rawRecommendations: RawRecommendation[] = [];
    ...
    for (let i = 0; i < coreSongs.length; i++) {
      ...
      if (sessionId) {
        sendProgress(
          ...
          i + 1,
          coreSongs.length,
          `Fetching recommendations for "${title}"...`
        );
      }

      console.log(
        `[Recommendations] Fetching ${quota} similar tracks for "${title}" by ${artists}`
      );
      ...
      try {
        ...
        const now = new Date();
        let similarTracks: Array<{ title: string; artists: string; score: number }>;
        ...
        console.log(
          `[Recommendations] Added ${added}/${quota} raw tracks for "${title}"`
        );
      } catch (error) {
        console.error(
    ...
    const uniqueTracks = new Map<string, RawRecommendation>();
    ...
    for (const [_key, track] of uniqueTracks) {
      try {
        ...
        if (searchCount % 5 === 0) {
          ...
          if (sessionId) {
            sendProgress(
              ...
              coreSongs.length,
              coreSongs.length,
              `Searching Spotify: ${searchCount}/${uniqueTracks.size} tracks...`
            );
          }
        }
        ...
        console.error(
          `[Recommendations] Error searching Spotify for "${track.title}":`,
          error
        );
      }
    }
    ...
    console.log(
      `[Recommendations] Returning ${allRecommendations.length} total recommendations`
    );
    ...
    if (sessionId) {
      ...
      sendProgress(
        ...
        coreSongs.length,
        coreSongs.length,
        `Complete! Found ${allRecommendations.length} recommendations`
      );
      ...
      if (connection) {
        connection.write(
          `data: ${JSON.stringify({ type: "complete", recommendations: allRecommendations.length })}\n\n`
        );
    ...
    if ((req.body as any).sessionId) {
      const connection = activeConnections.get((req.body as any).sessionId);
      if (connection) {
        connection.write(
