...
import { syncController } from '../services/sync.js';
...
router.post('/smart', async (req, res) => {
  try {
    ...
    const result = await syncController.smartSync(userId, clientTimestamp, data);
...
router.post('/upload', async (req, res) => {
  try {
    ...
    const serverTimestamp = await syncController.uploadSyncData(userId, {
      songs,
      playlists,
      recommendations,
...
router.get('/restore/:userId', async (req, res) => {
  try {
    ...
    const data = await syncController.getSyncData(userId);
...
router.get('/status/:userId', async (req, res) => {
  try {
    ...
    const status = await syncController.getSyncStatus(userId);
...
router.delete('/:userId', async (req, res) => {
  try {
    ...
    await syncController.deleteSyncData(userId);
