...
import { SimilarTracksCache } from "../models/SimilarTracksCache";
...
// Store recommendation processing state
...
interface SimilarTrack {
  ...
  artists: string;
  score: number;
  // Full track details (enriched after finding recommendations)
  album?: string;
  durationMs?: number;
  artwork?: any;
  ext?: any;
 ...
 */
async function fetchFullTrackDetails(trackId: string): Promise<any | null> {
  try {
    const trackData = await spotifyClient.getTrack(trackId, 'US') as any;
    ...
    return {
      ...
      ext: {
        spotify: {
          ...
          artistIds: trackData.artists?.map((a: any) => a.id).filter(Boolean) || [],
...
router.post("/similar", async (req: Request, res: Response) => {
  const { likedSongs, dislikedSongIds = [], likedSongIds = [], sessionId } = req.body as RecommendationRequest & { sessionId?: string };
  ...
  console.log(
    `[Recommendations] Started processing ${likedSongs.length} liked songs (session: ${sessionId})`
  );
...
async function processRecommendations(
  sessionId: string,
  likedSongs: RecommendationRequest['likedSongs'],
  dislikedSongIds: string[],
  likedSongIds: string[]
) {
  try {
    ...
    const availableSongs: typeof likedSongs = [];
    const unavailableSongs: typeof likedSongs = [];
    ...
    for (let i = 0; i < sortedAllSongs.length; i++) {
      ...
      // Check cache first
      const cached = await SimilarTracksCache.findOne({ trackId: song.id });
      const now = new Date();
      ...
      if (cached && cached.expiresAt > now && cached.similarTracks && cached.similarTracks.length > 0) {
        ...
        try {
          ...
          const lastfmTracks = await LastFmService.getSimilarTracks(
            song.artists,
            song.title,
            10 // Just check if it exists, don't need all 50
          );
    ...
    while (totalQuota > totalRecommendations && coreSongs.length > 0) {
      ...
      if (lastWithMoreThan1) lastWithMoreThan1.quota--;
      coreSongs.reverse();
      totalQuota--;
    }
    ...
    const allRecommendations: SimilarTrack[] = [];
    ...
    for (let i = 0; i < coreSongs.length; i++) {
      ...
      console.log(
        `[Recommendations] [${i + 1}/${coreSongs.length}] Fetching ${quota} similar tracks for "${title}" by ${artists}`
      );

      try {
        // Check cache first
        const cached = await SimilarTracksCache.findOne({ trackId: id });
        const now = new Date();
        let similarTracks: Array<{ title: string; artists: string; score: number; sourceArtist: string }>;
        ...
        if (cached && cached.expiresAt > now) {
          ...
          const lastfmTracks = await LastFmService.getSimilarTracks(
            artists,
            title,
            50 // Get 50 tracks to build a good cache
          );
          ...
          await SimilarTracksCache.findOneAndUpdate(
            { trackId: id },
            {
              trackId: id,
              artist: artists,
        ...
        const tracksByArtist = new Map<string, typeof similarTracks>();
        ...
        const artistIndexes = new Map<string, number>();
        ...
        while (added < quota && attempts < maxAttempts) {
          ...
          try {
            ...
            const existingInDb = await Song.findOne({
              $or: [
            ...
            const results = await spotifyClient.searchTracks(query, 1);
        ...
        console.log(
          `[Recommendations] Added ${added}/${quota} tracks for "${title}" (${sortedArtists.length} artists, ${attempts} attempts)`
        );
        
      } catch (error) {
        console.error(
          `[Recommendations] ❌ Error fetching recommendations for "${title}":`,
          error
        );
    ...
    const enrichedRecommendations: SimilarTrack[] = [];
