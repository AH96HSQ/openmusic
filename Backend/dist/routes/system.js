...
router.get("/info", async (req, res, next) => {
    try {
        logger_1.log.search(`System info request from: ${req.ip}`);
        const systemInfo = await (0, systemMonitor_1.getSystemMetrics)();
        logger_1.log.success(`System info collected: ${systemInfo.memory.usagePercent}% RAM, ${systemInfo.cpu.currentUsage}% CPU`);
        return res.json({
            ok: true,
            system: systemInfo
        });
        ...
        logger_1.log.error(`System info error: ${err.message}`);
        return next(err);
    }
});
/**
...
router.get("/info/formatted", async (req, res, next) => {
    try {
        logger_1.log.search(`Formatted system info request from: ${req.ip}`);
        const systemInfo = await (0, systemMonitor_1.getSystemMetrics)();
        const formatted = {
            timestamp: systemInfo.timestamp,
            memory: {
                total: (0, systemMonitor_1.formatBytes)(systemInfo.memory.total),
                used: (0, systemMonitor_1.formatBytes)(systemInfo.memory.used),
                free: (0, systemMonitor_1.formatBytes)(systemInfo.memory.free),
            ...
            storage: {
                drives: systemInfo.storage.drives.map(drive => ({
                    drive: drive.drive,
                    total: (0, systemMonitor_1.formatBytes)(drive.total),
                    used: (0, systemMonitor_1.formatBytes)(drive.used),
                    free: (0, systemMonitor_1.formatBytes)(drive.free),
                    usagePercent: `${drive.usagePercent}%`
                })),
                totalStorage: (0, systemMonitor_1.formatBytes)(systemInfo.storage.totalStorage),
                totalUsed: (0, systemMonitor_1.formatBytes)(systemInfo.storage.totalUsed),
                totalFree: (0, systemMonitor_1.formatBytes)(systemInfo.storage.totalFree),
            ...
            system: {
                ...
                uptime: (0, systemMonitor_1.formatUptime)(systemInfo.system.uptime),
        ...
        logger_1.log.success(`Formatted system info collected`);
        return res.json({
            ok: true,
            system: formatted
        });
        ...
        logger_1.log.error(`Formatted system info error: ${err.message}`);
        return next(err);
    }
});
/**
...
router.get("/memory", async (req, res, next) => {
    try {
        ...
        return res.json({
            ...
            memory: {
                ...systemInfo.memory,
                totalFormatted: (0, systemMonitor_1.formatBytes)(systemInfo.memory.total),
                usedFormatted: (0, systemMonitor_1.formatBytes)(systemInfo.memory.used),
                freeFormatted: (0, systemMonitor_1.formatBytes)(systemInfo.memory.free)
            }
        });
    }
    catch (err) {
        logger_1.log.error(`Memory info error: ${err.message}`);
        return next(err);
    }
});
/**
...
router.get("/cpu", async (req, res, next) => {
    try {
        ...
        logger_1.log.error(`CPU info error: ${err.message}`);
        return next(err);
    }
});
/**
...
router.get("/storage", async (req, res, next) => {
    try {
        ...
        return res.json({
            ...
            storage: {
                ...systemInfo.storage,
                totalStorageFormatted: (0, systemMonitor_1.formatBytes)(systemInfo.storage.totalStorage),
                totalUsedFormatted: (0, systemMonitor_1.formatBytes)(systemInfo.storage.totalUsed),
                totalFreeFormatted: (0, systemMonitor_1.formatBytes)(systemInfo.storage.totalFree),
                drives: systemInfo.storage.drives.map(drive => ({
                    ...drive,
                    totalFormatted: (0, systemMonitor_1.formatBytes)(drive.total),
                    usedFormatted: (0, systemMonitor_1.formatBytes)(drive.used),
                    freeFormatted: (0, systemMonitor_1.formatBytes)(drive.free)
                }))
            }
        });
    }
    catch (err) {
        logger_1.log.error(`Storage info error: ${err.message}`);
        return next(err);
    }
});
/**
...
router.get("/health", async (req, res, next) => {
    try {
        ...
        logger_1.log.error(`System health error: ${err.message}`);
        return next(err);
    }
});
/**
...
router.get("/database", async (req, res, next) => {
    try {
        ...
        logger_1.log.error(`Database stats error: ${err.message}`);
        return next(err);
    }
});
/**
...
router.get("/donation", async (req, res, next) => {
    try {
        logger_1.log.search(`Donation info request from: ${req.ip}`);
        ...
        logger_1.log.success(`Donation info collected: ${Math.round(donationInfo.progress * 100)}% of goal`);
        return res.json({
            ok: true,
        ...
        logger_1.log.error(`Donation info error: ${err.message}`);
        return next(err);
    }
});
/**
...
router.post("/donation/crypto", async (req, res, next) => {
    try {
        logger_1.log.search(`Update crypto addresses request from: ${req.ip}`);
        ...
        logger_1.log.success(`Crypto addresses updated successfully`);
        return res.json({
            ok: true,
        ...
        logger_1.log.error(`Update crypto addresses error: ${err.message}`);
        return next(err);
    }
});
/**
...
router.get("/version", async (req, res, next) => {
    try {
        logger_1.log.search(`Version check request from: ${req.ip}`);
        const versionInfo = {
            latest: "0.2.0",
        ...
        logger_1.log.success(`Version info returned: ${versionInfo.latest}`);
        return res.json({
            ok: true,
            version: versionInfo
        });
        ...
        logger_1.log.error(`Version info error: ${err.message}`);
        return next(err);
    }
});
