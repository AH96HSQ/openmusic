...
router.get("/spotify-credentials", async (_req, res, next) => {
    try {
        ...
        if (!clientId || !clientSecret) {
            logger_1.log.info("Config: No public Spotify credentials configured, app will use defaults");
            res.status(404).json({
        ...
        logger_1.log.info("Config: Returning public Spotify credentials");
        res.json({
            clientId,
            clientSecret,
...
router.get("/download-credentials", async (_req, res, next) => {
    try {
        ...
        if (!rapidApiKey && !dsRapidApiKey) {
            logger_1.log.info("Config: No public download credentials configured, app will use defaults");
            res.status(404).json({
        ...
        logger_1.log.info("Config: Returning public download credentials");
        res.json({
