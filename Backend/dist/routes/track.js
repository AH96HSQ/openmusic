...
router.get("/:id/details", async (req, res, next) => {
    try {
        ...
        let song = await Song_1.Song.findById(id).lean();
        ...
        if (song && song.ext?.spotify?.albumId && song.ext?.spotify?.artistIds?.length) {
            ...
            logger_1.log.search(`Track details (cached): ${id}`);
        ...
        logger_1.log.search(`Track details (fetching): ${id}`);
        const trackData = await spotify_1.spotifyClient.getTrack(id, market);
        ...
        const artistIds = trackData.artists?.map((artist) => artist.id).filter(Boolean) || [];
        ...
        const enhancedTrackData = {
            ...
            artists: trackData.artists?.map((a) => a.name || "").filter(Boolean) || [],
        ...
        const mappedSong = (0, spotifyMapper_1.mapSpotifyTrackToSong)(enhancedTrackData);
        ...
        const savedSong = await Song_1.Song.findByIdAndUpdate(mappedSong._id, mappedSong, { upsert: true, new: true });
        console.log(`albums and artists playback: Successfully saved song to database: ${savedSong._id}`);
        logger_1.log.search(`Track enriched and saved: ${id}`);
        // Format to match search results structure
        const fileInfo = { exists: false }; // TODO: Add file check if needed
        const response = {
            source: "spotify",
